...
class BatchSpanProcessorBase {
    constructor(_exporter, config) {
        ...
        this._exportTimeoutMillis =
            typeof (config === null || config === void 0 ? void 0 : config.exportTimeoutMillis) === 'number'
                ...
                console.log("max queue size: ", this._maxQueueSize);
                console.log("delay: " , this._scheduledDelayMillis);
                console.log("export timeout", this._exportTimeoutMillis);
                console.log("export batch size", this._maxExportBatchSize);
    }
    forceFlush() {
        if (this._isShutdown) {
    ...
    _addToBuffer(span) {
        if (this._finishedSpans.length >= this._maxQueueSize) {
            ...
            this._droppedSpans++;
            console.log("span dropped from: " + span.spanContext().traceId);
            return;
        }
    ...
    _flushOneBatch() {
        console.log("I AM FLUSHING A BATCH")
        if (this._droppedSpans > 0 ) {
          const droppedSpans = this._droppedSpans;
          this._droppedSpans = 0;
          console.log("FYI, " + droppedSpans + " were dropped on the floor");
        }
        this._clearTimer();
